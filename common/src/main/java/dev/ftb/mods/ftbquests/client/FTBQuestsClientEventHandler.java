...
import dev.ftb.mods.ftbquests.client.gui.MyCustomScreen;
...
public class FTBQuestsClientEventHandler {
	...
	private void collectAllQuestsWithProgress(ClientQuestFile file) {
		TeamData data = file.selfTeamData;

		List<Quest> allQuests = new ArrayList<>();
		file.forAllQuests(allQuests::add);  // Добавляем все квесты в список

		Minecraft mc = Minecraft.getInstance();

		pinnedQuestText.clear(); // Можно переиспользовать эту же переменную или создать новую

		for (int i = 0; i < allQuests.size(); i++) {
			Quest quest = allQuests.get(i);

			if (i > 0) pinnedQuestText.add(FormattedCharSequence.EMPTY);  // Разделитель
